...
async function main() {
  ...
  check(
    "creator gets a 6-char invite code",
    /^[A-Z2-9]{6}$/.test(joinedA.code),
    joinedA.code,
  );
  check(
    "creator gets a private token",
    typeof joinedA.token === "string" && joinedA.token.length >= 40,
  );
  ...
  const stateMsg = await a.wait(
    (m) => m.type === "state" && m.state.players.yellow,
  );
  const stateA = stateMsg.state;
  check(
    "state never leaks a token",
    !JSON.stringify(stateMsg).includes(joinedA.token),
  );
  check(
    "both player names are present",
    stateA.players.red.name === "Alice" && stateA.players.yellow.name === "Bob",
  );
  ...
  check(
    "a string column is refused",
    errStr.code === "invalid_column",
    errStr.code,
  );
    ...
    finalState = (
      await a.wait(
        (m) =>
          m.type === "state" &&
          m.state.lastMove &&
          m.state.lastMove.col === col,
      )
    ).state;
  }

  check(
    "a vertical connect-four ends the game",
    finalState.status === "won",
    finalState.status,
  );
  check(
    "red is recorded as the winner",
    finalState.winner === 1,
    String(finalState.winner),
  );
  check(
    "four winning cells are reported",
    finalState.winningCells.length === 4,
  );
  ...
  a.send({ type: "rematch" });
  const afterOne = (
    await a.wait((m) => m.type === "state" && m.state.rematchVotes.length === 1)
  ).state;
  check(
    "one rematch vote does not restart the game",
    afterOne.status === "won",
  );
  ...
  b.send({ type: "rematch" });
  const afterBoth = (
    await a.wait((m) => m.type === "state" && m.state.status === "playing")
  ).state;
  check("both votes restart the game", afterBoth.status === "playing");
  check(
    "the board is cleared on rematch",
    afterBoth.board.every((r) => r.every((c) => c === 0)),
  );
  check(
    "the loser opens the rematch",
    afterBoth.turn === 2,
    String(afterBoth.turn),
  );
  ...
  check(
    "a third client is downgraded to spectator",
    joinedC.seat === "spectator",
    joinedC.seat,
  );
  ...
  check(
    "a forged token does not hijack a seat",
    joinedD.seat === "spectator",
    joinedD.seat,
  );
  ...
  check(
    ...
    (await e.wait(isType("error"))).code === "bad_json",
  );

  e.ws.send("[1,2,3]");
  check(
    "a non-object payload is rejected",
    (await e.wait(isType("error"))).code === "bad_message",
  );
  ...
  check(
    ...
    (await e.wait(isType("error"))).code === "unknown_type",
  );

  e.send({ type: "join", code: "../../etc/passwd" });
  check(
    "a path-traversal room code is rejected",
    (await e.wait(isType("error"))).code === "bad_code",
  );

  e.send({ type: "join", code: "ZZZZZZ" });
  check(
    "an unknown room code is rejected",
    (await e.wait(isType("error"))).code === "no_such_room",
  );

  e.send({ type: "move", column: 0 });
  check(
    "moving without a room is rejected",
    (await e.wait(isType("error"))).code === "not_in_room",
  );

  check(
    "the connection survived every malformed payload",
    e.ws.readyState === WebSocket.OPEN,
  );

  // --- prototype pollution attempt ---
  e.ws.send(
    JSON.stringify({ type: "join", code, __proto__: { polluted: true } }),
  );
  ...
  try {
    ...
    /* no rate-limit message arrived */
  }
  check("a message flood is rate limited", limited);
  check(
    "the flooding connection stays open (throttled, not killed)",
    f.ws.readyState === WebSocket.OPEN,
  );
  ...
  const closedBig = new Promise((res) =>
    g.ws.once("close", (codeNum) => res(codeNum)),
  );
  ...
  check(
    "a foreign Origin is refused (CSWSH)",
    evilResult !== "open",
    String(evilResult),
  );
  ...
  check(
    "the real token reclaims the original seat",
    rejoined.seat === "red",
    rejoined.seat,
  );
  ...
  a2.send({ type: "chat", text: "good game" });
  const chatB = await b.wait((m) => m.type === "chat");
  check(
    "a message reaches the opponent",
    chatB.message.text === "good game",
    chatB.message.text,
  );
  check(
    "the message is attributed to the right seat",
    chatB.message.seat === "red",
    chatB.message.seat,
  );
  check(
    "the sender name comes from the server",
    chatB.message.name === "Alice",
    chatB.message.name,
  );

  const chatC = await c.wait((m) => m.type === "chat");
  check("spectators receive chat too", chatC.message.text === "good game");

  // A client must not be able to speak as someone else.
  b.drain();
  a2.send({ type: "chat", text: "impersonation", name: "Bob", seat: "yellow" });
  const spoof = await b.wait((m) => m.type === "chat");
  check(
    "a client cannot forge the chat name",
    spoof.message.name === "Alice",
    spoof.message.name,
  );
  check(
    "a client cannot forge the chat seat",
    spoof.message.seat === "red",
    spoof.message.seat,
  );

  // XSS payload must survive as text, intact, so the client renders it inert.
  b.drain();
  const payload = "<img src=x onerror=alert(1)>";
  a2.send({ type: "chat", text: payload });
  const xss = await b.wait((m) => m.type === "chat");
  check(
    "an XSS payload is delivered as literal text",
    xss.message.text === payload,
    xss.message.text,
  );

  // Spectators can talk, and are labelled as such.
  b.drain();
  c.send({ type: "chat", text: "nice one" });
  const fromSpec = await b.wait((m) => m.type === "chat");
  check(
    "a spectator message is labelled spectator",
    fromSpec.message.seat === "spectator",
    fromSpec.message.seat,
  );

  // Empty / non-string messages are refused.
  // The chat bucket is consumed BEFORE validation (so malformed input cannot
  // burn CPU on normalisation), so let it refill or these hit the throttle
  // instead of the validator and the test would assert the wrong thing.
  await new Promise((r) => setTimeout(r, 4000));
  a2.drain();
  a2.send({ type: "chat", text: "   " });
  check(
    "a whitespace-only message is refused",
    (await a2.wait(isType("error"))).code === "bad_chat",
  );
  a2.send({ type: "chat", text: 12345 });
  check(
  ...
  a2.send({ type: "chat" });
  check(
    "a missing message body is refused",
    (await a2.wait(isType("error"))).code === "bad_chat",
  );

  // Chat has its own, tighter throttle.
  a2.drain();
  for (let i = 0; i < 30; i++) a2.send({ type: "chat", text: `spam ${i}` });
  ...
  check("chat flooding is rate limited", chatLimited);
  check(
    "the flooding client stays connected",
    a2.ws.readyState === WebSocket.OPEN,
  );

  // A message longer than the cap is truncated, not rejected outright.
  // Wait BEFORE draining: the flood above is still being delivered, and a
  // drain that runs first just leaves the stragglers in the queue to be
  // mistaken for this message.
  await new Promise((r) => setTimeout(r, 2000));
  b.drain();
  a2.drain();
  a2.send({ type: "chat", text: "y".repeat(2000) });
  const longMsg = await b.wait((m) => m.type === "chat", 5000);
  check(
    "an over-long message is truncated to the cap",
    longMsg.message.text.length === 300,
    String(longMsg.message.text.length),
  );

  // A late joiner gets the backlog.
  const late = client();
  await late.open();
  await late.wait(isType("hello"));
  late.send({ type: "join", code, name: "Latecomer" });
  const history = await late.wait(isType("chat_history"));
  check(
  ...
  check(
    "history contains no token field",
